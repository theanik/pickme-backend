	...
	switch(request.url){
		...
		case '/generate_checksum':
			if(request.method == 'POST'){
				...
				console.log(request.body.customer_id)
				console.log(request.body.amount)
			var paramarray = {};
				paramarray['MID'] = paytm_config.MID //'xxxxxxxxxxxxxx'; //Provided by Paytm
				paramarray['ORDER_ID'] = request.body.order_id //'ORDER00001'; //unique OrderId for every request
				paramarray['CUST_ID'] = request.body.customer_id //'CUST0001';  // unique customer identifier 
				paramarray['INDUSTRY_TYPE_ID'] = paytm_config.INDUSTRY_TYPE_ID //'xxxxxxxxx'; //Provided by Paytm
				paramarray['CHANNEL_ID'] = paytm_config.CHANNEL_ID //'WAP'; //Provided by Paytm
				paramarray['TXN_AMOUNT'] = request.body.amount //'1.00'; // transaction amount
				paramarray['WEBSITE'] = paytm_config.WEBSITE //'xxxxxxxxxxxx'; //Provided by Paytm
		...
		case '/verify_checksum':
			if(request.method == 'POST'){
				...
				request.on('end', function() {
					...
					// remove this f rombody, will be passed to function as separate argument
